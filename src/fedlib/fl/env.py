...
class simulator(ABC):
    def __init__(self, config_client, config_server) -> None:
        super().__init__()
        client_args = config_client
        server_args = config_server
        n_clients = config_client.get("n_clients")
    ...
    def client_updates(self, **kwargs):
        self.client.client_update(**kwargs)

    def server_update(self, **kwargs):
        self.server.server_update(**kwargs)
