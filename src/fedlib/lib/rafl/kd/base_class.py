...
class BaseClass:
    ...
    #         data = data.to(self.device)
    #         teacher_out.append(self.teacher_model(data))
    #     return teacher_out
    ...
    def _train_student(
        ...
        # TODO: calculate the output outside the ep for loop (under test)
        # teacher_out = self.calculate_labels()
        for ep in range(epochs):
            epoch_loss = 0.0
            correct = 0
            lr = adjust_learning_rate(self.optimizer_student, ep, self.lr, epochs, lr_type='cos')
            for i, (data, label) in enumerate(self.train_loader):
    ...
    def train_student(
        ...
        :param save_model_pth (str): Path where you want to save the student model
        """
        lr = self._train_student(epochs, plot_losses, save_model, save_model_pth)
        return lr
