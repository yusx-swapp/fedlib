...
def compute_metrics(eval_pred: EvalPrediction,tokenizer):
    ...
    f1 = sum(compute_f1(pred, label) for pred, label in zip(decoded_preds, decoded_labels)) / len(decoded_preds)
    em = sum(compute_exact_match(pred, label) for pred, label in zip(decoded_preds, decoded_labels)) / len(decoded_preds)
...
def __prepare_features_for_t5(examples, tokenizer):
    max_length = 384
    pad_to_max_length = True

    # Format the input as T5 expects
    input_pairs = ["question: " + q + " context: " + c for q, c in zip(examples['question'], examples['paragraph'])]
    inputs = tokenizer(input_pairs, padding="max_length", max_length=max_length, truncation=True, return_tensors="pt")
...
def main(args):
    ...
    if args.dataset == "squad":
        ...
        # datasets = load_dataset('lmqg/qg_squad')
...
#CUDA_VISIBLE_DEVICES=2 python fl_t5_squad.py --algo vanilla --save_model --split_data --num_clients 100 --num_rounds 100 --num_local_epochs 3 --per_device_train_batch_size 8 --per_device_eval_batch_size 8 --dataset squad --log_dir log_t5 --model t5 > baseline_t5_100.txt

#python fl_t5_squad.py --model t5 --spp --split_data --num_clients 100 --num_rounds 100 --num_local_epochs 3 --per_device_train_batch_size 8 --per_device_eval_batch_size 8 --dataset squad --log_dir log_small > raffm_t5small_squadv1_100.txt
#python fl_t5_squad.py --model t5-base --spp --split_data --num_clients 100 --num_rounds 100 --num_local_epochs 3 --per_device_train_batch_size 8 --per_device_eval_batch_size 8 --dataset squad --log_dir log_base> raffm_t5base_squadv1_100.txt
