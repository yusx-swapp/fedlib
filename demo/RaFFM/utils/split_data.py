...
#     def __init__(self, dataset, seed=None):
#         self.dataset = dataset
#         self.seed = seed if seed is not None else np.random.randint(1e8)
...
#         sub_datasets = []
#         for _ in range(n):
#             indices = np.random.choice(len(self.dataset), size, replace=True)
...
#         size = len(self.dataset) // n
#         sub_datasets = [Subset(self.dataset, indices[i*size:(i+1)*size]) for i in range(n)]
#         # add remaining elements to the last sub-dataset
#         if len(self.dataset) % n != 0:
#             sub_datasets[-1] = Subset(self.dataset, list(sub_datasets[-1].indices) + list(indices[n*size:]))
#         return sub_datasets
